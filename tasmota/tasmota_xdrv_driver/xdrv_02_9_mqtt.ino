  ...
  if (prefix == "tele")
  {
    if (attr == "SENSOR")
    {
      ...
      for (auto sensorKey : payloadRoot)
      {
        ...
        // Special handling for ZbReceived data which has nested structure
        if (strcmp(sensorName, "ZbReceived") == 0)
        {
          for (auto deviceKey : sensors)
          {
            const char *deviceId = deviceKey.getStr();
            JsonParserObject deviceData = deviceKey.getValue().getObject();

            if (deviceData)
            {
              for (auto deviceAttr : deviceData)
              {
                const char *attrName = deviceAttr.getStr();
                JsonParserToken attrValue = deviceAttr.getValue();

                char formattedKey[100];
                char formattedValue[100];

                // Format the key as ZbReceived-DeviceID-AttributeName
                snprintf_P(formattedKey, sizeof(formattedKey), PSTR("%s-%s-%s"), sensorName, deviceId, attrName);

                if (attrValue.isNull())
                {
                  // Include null values as empty strings
                  strcpy(formattedValue, "\"\"");
                }
                else if (attrValue.isStr())
                {
                  // Quote string values
                  snprintf_P(formattedValue, sizeof(formattedValue), PSTR("\"%s\""), attrValue.getStr());
                }
                else
                {
                  // Use raw value for numbers and booleans
                  strlcpy(formattedValue, attrValue.getStr(), sizeof(formattedValue));
                }
                ...
                newPayload += "\"" + String(formattedKey) + "\":" + String(formattedValue);
              }
            }
          }
        }
        ...
        {
          // Original code for other sensor types
