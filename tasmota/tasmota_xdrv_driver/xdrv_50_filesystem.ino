...
void UfsInitOnce(void)
{
  ...
  // try lfs first
  ffsp = &LittleFS;
  if (!LittleFS.begin(true, "") && !LittleFS.begin(true, "", 5, "fs_1"))
  { // force empty mount point to make it the fallback FS
    // ffat is second
    ffsp = &FFat;
    if (!FFat.begin(true, ""))
    {
...
void UfsCheckSDCardInit(void)
{
  ...
  else if (PinUsed(GPIO_SDIO_CLK) && PinUsed(GPIO_SDIO_CMD) && PinUsed(GPIO_SDIO_D0))
  {
    ...
    if (SD_MMC.begin("/sd", !bit_4_mode /*mode 1 bit*/, false /*format_if_failed*/))
    { // mount under "/sd" to be consistent with SD SPI
      ufsp = &SD_MMC;
...
uint8_t UfsReject(char *name)
{
  ...
  if (!strncasecmp(name, "SPOTLI~1", REJCMPL))
  {
    return 1;
  }
  if (!strncasecmp(name, "TRASHE~1", REJCMPL))
  {
    return 1;
  }
  if (!strncasecmp(name, "FSEVEN~1", REJCMPL))
  {
    return 1;
  }
  if (!strncasecmp(name, "SYSTEM~1", REJCMPL))
  {
    return 1;
  }
  if (!strncasecmp(name, "System Volume", 13))
  {
    return 1;
  }
  return 0;
...
bool TfsLoadFile(const char *fname, uint8_t *buf, uint32_t len)
{
  ...
  {
    len = flen;
  } // Adjust requested length to smaller file length
  file.read(buf, len);
  file.close();
  return true;
}
...
void FileLoggingAsync(bool refresh)
{
  ...
  if (!ffs_type || !filelog_level)
  {
    return;
  } // No filesystem or [FileLog] disabled
  if (refresh && !NeedLogRefresh(filelog_level, index))
  {
    return;
  } // No log buffer changes
  ...
  for (uint32_t retry = 0; retry <= 1; retry++)
  {
    ...
    if (!fs_full && (file.size() < (FILE_LOG_SIZE * 1000)))
    {
      break;
    }
    ...
    if ((1 == filelog_option) &&
        (fs_full || (log_file_idx == FILE_LOG_COUNT)))
    {                              // Rotate until free space is less than FILE_LOG_FREE
      Settings->filelog_level = 0; // [FileLog] disable
      AddLog(LOG_LEVEL_INFO, PSTR("FLG: Logging disabled. Max rotates"));
      return;
    }

    if (log_file_idx >= FILE_LOG_COUNT)
    {
      log_file_idx = 0;
    } // Rotate max 16 log files
    ...
    { // Remove oldest log file(s)
      ...
      do
      { // Need free space around FILE_LOG_SIZE so find oldest log file(s) and remove it
        snprintf_P(fname, sizeof(fname), PSTR(FILE_LOG_NAME), idx + 1);
        if (ffsp->remove(fname))
        {                                                                       // Remove oldest (non-)sequential log file(s)
          AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("FLG: Delete file %s"), fname + 1); // Skip leading slash
        }
        idx++;
        if (idx >= FILE_LOG_COUNT)
        {
          idx = 0;
        }
...
{
  if (UfsExecuteCommandFileReady() || !ffs_type)
  {
    return;
  }
...
bool _UfsJsonSettingsUpdate(const char *data)
{
  ...
  if (p)
  {
    ...
    {
      return false;
    } // Error - No valid key provided in data to append
...
{
public:
  StaticRequestHandlerAuth(FS &fs, const char *path, const char *uri, const char *cache_header, bool requireAuth) : StaticRequestHandler(fs, path, uri, cache_header)
  {
...
bool isSDC(void)
{
  ...
  return false;
#else
  if (((uint32_t)ufsp != (uint32_t)ffsp) && ((uint32_t)ffsp == (uint32_t)dfsp))
    return false;
  if (((uint32_t)ufsp == (uint32_t)ffsp) && (ufs_type != UFS_TSDC))
    return false;
  return true;
#endif
}
